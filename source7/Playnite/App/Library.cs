...
public partial class Library : IDisposable
{
    ...
    public Library(string libraryDir)
    {
        ...
        // TODO: Games.ItemsRemoved += (_, e) => UpdateFieldsInUse(e.Items);
    ...
    public HashSet<string> GetImportedRomFiles(string emulatorDir)
    {
        ...
        foreach (var game in Games)
        {
            try
            {
                foreach (var rom in game.Variants?.Where(a => a.Roms.HasItems()).SelectMany(a => a.Roms!)!)
                {
    ...
    public HashSet<string> GetImportedExeFiles()
    {
        ...
        foreach (var game in Games)
        {
            ...
            var actionIds = game.Variants?.
                Where(a => a.IsInstalled && a.GameActions.HasItems()).
                SelectMany(a => a.GameActions!);
            if (actionIds == null)
            {
                continue;
            }
            ...
            foreach (var action in actions.Where(a => !a.Path.IsNullOrWhiteSpace()))
            {
                ...
                imported.Add(action.Path + action.Arguments ?? string.Empty);
            }
        }
